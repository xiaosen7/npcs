...
export const TagsEditor = <TSearchedTag extends ITag>(
  ...
  return mp(
    ...
    <div className="relative">
      <div
        ref={containerRef}
        className="light-border-2 text-dark300_light700 background-light900_dark300 paragraph-regular dark:bg-background flex border"
      >
      ...
      {open && (
        <div className="bg-light-800 dark:bg-dark-400 absolute top-full z-10 mt-3 w-full rounded-lg shadow-sm">
          {loading ? (
            ...
            <div className="background-light900_dark200 flex max-h-[60vh] flex-wrap overflow-auto p-2">
              {data.length > 0 ? (
                data.map((item) => (
                  ...
                  >
                    <TagCard
                      className="hover:bg-light-800 dark:hover:bg-background rounded-sm border-none shadow-none"
                      linkable={false}
                      tag={item}
                      totalQuestions={
