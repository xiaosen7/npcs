...
for APP_DIR in apps/*/; do
    ...
    # 获取 turbo build hash
    APP_TURBO_HASH=$(pnpm turbo build --filter="$APP_PACKAGE_NAME" --dry=json | jq -r --arg PACKAGE_NAME "$APP_PACKAGE_NAME" '.tasks[] | select(.package==$PACKAGE_NAME and .task=="build") | .hash')
